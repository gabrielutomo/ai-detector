...
async def predict(file: UploadFile = File(...)):
    ...
    # Reload model if improved (for dev/live updates)
    global model
    if MODEL_PATH.exists():
        # Simple reload check could be added here, or just reload if model is stale
        # For now, we'll reload it to ensure we have the absolute latest from training
        try:
            model = tf.keras.models.load_model(str(MODEL_PATH))
        except:
            pass
